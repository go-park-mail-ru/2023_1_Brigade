...
func (r repository) CreateSession(ctx context.Context, session model.Session) error {
	_, err := r.db.QueryxContext(ctx, `INSERT INTO session (cookie, profile_id) VALUES ($1, $2)`, session.Cookie, session.UserId)
	return err
}

func (r repository) DeleteSession(ctx context.Context, cookie string) error {
	_, err := r.db.QueryxContext(ctx, "DELETE FROM session WHERE cookie=$1", cookie)
