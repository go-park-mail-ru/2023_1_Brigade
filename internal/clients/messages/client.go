...
func (m messagesServiceGRPCClient) PutInProducer(ctx context.Context, producerMessage model.ProducerMessage) error {
	_, err := m.messagesClient.PutInProducer(ctx, model_conversion.FromProducerMessageToProtoProducerMessage(producerMessage))
	return err
}
