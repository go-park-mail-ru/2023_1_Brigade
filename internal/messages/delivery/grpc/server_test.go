...
import (
	...
	mockMessages "project/internal/messages/usecase/mocks"
...
func TestServer_PutInProducer_OK(t *testing.T) {
	...
	messagesUsecase.EXPECT().PutInProducer(context.TODO(), msg).Return(nil).Times(1)

	_, err := messagesSerivce.PutInProducer(context.TODO(), model_conversion.FromProducerMessageToProtoProducerMessage(msg))
