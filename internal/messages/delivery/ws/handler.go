...
func (u *messageHandler) SendMessagesHandler(ctx echo.Context) error {
	...
	//c := centrifuge.NewProtobufClient("", config) // url - адрес на котором работает centrifugo
	...
	//		return
	//	}
	//	log.Warn("Someone says via channel %s: %s (offset %d)", sub.Channel, chatMessage.Input, e.Offset)
	//})
