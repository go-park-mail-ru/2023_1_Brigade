...
func (r repository) CreateTechnogrammChat(ctx context.Context, user model.AuthorizedUser) error {
   ...
   VALUES ($1, $2, 'Привет, это технограмм - бесплатный мессенджер для обмена текстовыми сообщениями, фотографиями, эмодзи и стикерами.\n
                   Здесь ты можешь:\n
				   - Обмениваться фото, которые будут храниться во вложениях, текстовыми сообщениями, которые ты можешь редактировать и удалять, с одним или несколькими пользователями.\n
				   - Создавать групповой чат. В него ты можешь добавлять и удалять из него людей, которые есть у тебя в контактах.\n
				   - Создавать канал. На твой канал может подписаться любой пользователь, и при твоей публикации получит уведомление.\n
                   - Отправлять стикеры и эмодзи. Хасбик ждёт не дождётся, твоего сообщения с ним.\n
                   Желаем тебе удачного общения!',
           (SELECT id FROM chat WHERE id = $3), (SELECT id FROM profile WHERE id = $4), '0001-01-01 00:00:00+00');`, id, config.NotSticker, chat.Id, 0)
	if err != nil {
		_ = tx.Rollback()
		return err
	}
